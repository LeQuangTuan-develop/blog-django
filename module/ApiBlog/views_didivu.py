...
class BlogViewset(viewsets.ModelViewSet):
    queryset = Blog.objects.filter(
        is_valid=1, status_type=2, domain=DOMAIN_DIDIVU).order_by('-created_at')
...
class BlogHomeViewset(mixins.ListModelMixin, viewsets.GenericViewSet):
    queryset = Blog.objects.filter(
        is_valid=1, status_type=2, domain=DOMAIN_DIDIVU).order_by('-created_at')[:3]
    serializer_class = BlogSerializer
    permission_classes = [prod_per]
...
class BlogLikeCountViewSet(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    queryset = Blog.objects.filter(
        is_valid=1, domain=DOMAIN_DIDIVU).order_by('-created_at')
    serializer_class = BlogSerializer
...
class SeriesViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not user.is_superuser:  # type: ignore
            return BlogSeries.objects.filter(created_by=self.request.user, is_valid=1, domain=DOMAIN_DIDIVU).order_by('-created_at')
        return BlogSeries.objects.filter(is_valid=1, domain=DOMAIN_DIDIVU).order_by('-created_at')
    ...
    def destroy(self, request, *args, **kwargs):
        # check if request user is admin or author
        request_user = request.user
        target_reply = BlogSeries.objects.get(id=kwargs['pk'])
        if target_reply.created_by == request_user or request_user.is_superuser:
            target_reply.is_valid = 0
            target_reply.save()
...
class MyBlogViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        return Blog.objects.filter(created_by=user, domain=DOMAIN_DIDIVU).order_by('-created_at')
